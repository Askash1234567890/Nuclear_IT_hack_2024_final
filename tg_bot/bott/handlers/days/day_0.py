...
class MrBist:
    ...
    def _cleaning_answers(self, words):
        ...
        # в стоп-словах не было этих союзов
        reg_stop = '|'.join(
            map(lambda x: f'(^{x}$)', stopwords_ru)) + '|(^из-за$)|(^из-под$)|(^.$)'

        reg = r'[^А-Яа-я /-]'
        # обработаем плохие слова и стоп слова
        self.bad_reg = r'(.*ху.*)|(.*пиз.*)|(.*еба.*)|(.*еби.*)' + \
            rf'|{reg_stop}'
    ...
    def _lemmatize_answers(self, clean_words):
        ...
        for word in clean_words:
            ...
            if len(data) == 1:
                ...
                lemmas.append(' '.join([morph.parse(
                    w)[0].normal_form for w in data if not (re.search(self.bad_reg, w))]))
                if word:
                    self.dict_lemmas_to_words[lemmas[-1]] = word

        self.lemmas = [word for word in lemmas if not (
            re.search(self.bad_reg, word)) and word]
    ...
    def _count_clusters(self, clean_data):
        ...
        for n_clusters in tqdm(ran):
            try:
                ...
                centers, indices = kmeans_plusplus(
                    np.array(clean_data), n_clusters=n_clusters, random_state=10)
        ...
        x, y = np.array(range(inertia_arr.shape[0])), np.array(
            inertia_arr).flatten()
        y_norm = y.max()
        y /= y_norm
    ...
    def _describe_emotion(self, words):
        ...
        answers_with_emotions = {name: (val, mark) for name, val, mark in zip(
            answers, counter.values(), marks)}
    ...
    def get_bar_statistic(self, ans_info, title, index):
        ...
        ans_info = dict(
            sorted(ans_info.items(), key=lambda x: x[1][0], reverse=True))
        labels = []
        values = []
        emotions = []
        val_cnt = 0
    ...
    def get_pie_statistic(self, ans_info, title, index):
        ...
        # plt.show()
        plt.savefig(f'data/pie_{index}.png')
    ...
    def get_personal_statistic(self, id):
        ...
        prepared_info = {question: ans for question,
                         ans in zip(questions, short_ans)}

        df_info = pd.DataFrame.from_dict(prepared_info, orient='index', columns=[
